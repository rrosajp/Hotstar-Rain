...
def make_request(url):
    try:
        ...
        if enableip=='true':
            headers = {'User-Agent':'AppleCoreMedia/1.0.0.12B411 (iPhone; U; CPU OS 8_1 like Mac OS X; en_gb)', 'X-Forwarded-For': ipaddress, 'Referer': 'http://www.hotstar.com/'}
        else:
            headers = {'User-Agent':'AppleCoreMedia/1.0.0.12B411 (iPhone; U; CPU OS 8_1 like Mac OS X; en_gb)', 'Referer': 'http://www.hotstar.com/'}
...
def make_srequest(url):
    try:
        print 'enableip is', enableip
        if enableip=='true':
            headers = {'User-Agent':'AppleCoreMedia/1.0.0.12B411 (iPhone; U; CPU OS 8_1 like Mac OS X; en_gb)', 'X-Forwarded-For': ipaddress, 'Referer': 'http://www.hotstar.com/', 'Origin': 'http://www.hotstar.com'}
        else:
            headers = {'User-Agent':'AppleCoreMedia/1.0.0.12B411 (iPhone; U; CPU OS 8_1 like Mac OS X; en_gb)', 'Referer': 'http://www.hotstar.com/', 'Origin': 'http://www.hotstar.com'}
        # print 'url to fetch', url
        print 'headers are', headers
...
def premium_testing():
    ...
    if isit=="done":
        html = requests.get('https://secure-getcdn.hotstar.com/AVS/besc?action=GetCDN&asJson=Y&channel=APPLETV&type=VOD&id=1770001366', headers=headers, cookies=s.cookies, verify=False).json()
        # addon_log(html.text)
        cookieString=''
        c = s.cookies
        i = c.items()
...
def get_main_featured():
    ...
    for result in html['resultObj']['contentList']:
        ...
        show_link = 'https://secure-getcdn.hotstar.com/AVS/besc?action=GetCDN&asJson=Y&channel=PCTV&type=VOD&id='+str(result['contentId'])
...
def get_movies():
    ...
    for result in html['resultObj']['response']['docs']:
        title = '[B][COLOR orange]'+result['contentTitle'].encode('ascii','ignore')+'[/COLOR][/B]'
        duration = result['duration']
        movie_link = 'https://secure-getcdn.hotstar.com/AVS/besc?action=GetCDN&asJson=Y&channel=PCTV&type=VOD&id='+str(result['contentId'])
...
def get_ss_event():
    ...
    if 'ArrayContent' in url:
        for result in html['resultObj']['contentList']:
            title = result['contentTitle']
            duration = result['duration']
            event_link = 'https://secure-getcdn.hotstar.com/AVS/besc?action=GetCDN&asJson=Y&channel=PCTV&id='+str(result['contentId'])+'&type=VOD'
...
def col_movies():
    ...
    for result in html['resultObj']['contentList']:
        title = '[B][COLOR orange]'+result['contentTitle'].encode('ascii','ignore')+'[/COLOR][/B]'
        duration = result['duration']
        colm_link = 'https://secure-getcdn.hotstar.com/AVS/besc?action=GetCDN&asJson=Y&channel=PCTV&type=VOD&id='+str(result['contentId'])
...
def get_search():
    ...
    for result in html['resultObj']['response']['docs']:
        ...
        duration = result['duration']
        search_link = 'https://secure-getcdn.hotstar.com/AVS/besc?action=GetCDN&asJson=Y&channel=PCTV&type=VOD&id='+str(result['contentId'])
...
def get_episodes():
    ...
    for result in html['resultObj']['contentList']:
        ...
        duration = result['duration']
        fin_ep_links = 'https://secure-getcdn.hotstar.com/AVS/besc?action=GetCDN&asJson=Y&channel=PCTV&id='+str(result['contentId'])+'&type=VOD'
...
def get_video_url(url):
    ...
    addon_log(url)
    quality = (Addon.getSetting('qualityType')).lower()
    thtml = make_request('http://www.hotstar.com/get_cdn_token.php')
    token = json.loads(thtml)['token']
    url += '&hotstarauth=%s'%token
    html = make_srequest(url)
    data = html
    html = json.loads(data)
    manifest1 = html['resultObj']['src']
    addon_log('manifest1 is, '+manifest1)
